...
public class Anna implements Brain {
	...
	public Direction getNextMove(Snake snake, GameState gameState) {
		...
		System.out.println("Steps: " + stepsInTrail);
		//System.out.println(chosenDir);
		//System.out.println(d);
		
		//System.out.println("K: " + snake.getSegments().get(0));
	...
	public Direction main() {
		...
		// Direction chosenDir = hungryVsRiskDir(fruitDirs, leftTrail,
		// forwardTrail, rightTrail);
	...
	public void setOtherLivingSnakes() {
		//otherLivingSnakes = new ArrayList<Snake>();
	...
	public double calculateRisk(List<Trail> trails, int trailSteps) {
		...
		// System.out.println(areTailsGrowing);
		...
		for (Trail trail : tempList) {
					...
					for (int i = segments.size(); i > 0; i--) {
						...
						if (segmentPos.equals(squarePos)) {
							...
							if (turnsUntilSquareNotLeathal == segments.size()) {
								...
								if (trailSteps == 0) {
									totalRiskOfDeath = 1;
									//System.out.println("hejj2");
									break;
								} else if (trailSteps == 1) {
									totalRiskOfDeath = 0.999;
									//System.out.println("hopp2");
									break;
								} else {
									//System.out.println("TrailS: " + trailSteps);
			...
			if (!square.isLethal()) {
				...
				if (squarePositions.size() >= 5) { 
					for (int steps = 0; steps < squarePositions.size() - 4; steps++) {
						if (squarePositions.get(steps).equals(squarePos)
								&& snake.getSegments().size() >= squarePositions.size() - steps) { // Eller???
							//System.out.println("Har hander en grej...");
							riskOfDeath = 1;
							break;
						}
					}
				...
				if (turnsUntilSquareNotLeathal > trailSteps) {
					// System.out.println(turnsUntilSquareNotLeathal + " =
					// distans to snake");
					riskOfDeath = 1;
